  ...
  <div>
    ...
    <form class="mt-8 space-y-6" @submit="register">
      ...
      <div class="rounded-md shadow-sm -space-y-px flex flex-col gap-4">
        <didv>
          <label for="name" class="pl-.5 font-medium"> Full Name</label>
          <TInput name="name" v-model="user.name" :errors="errors" placeholder="Full Name" inputClass="rounded" />
        </didv>
        <div>
          <label for="email" class="pl-.5 font-medium"> Email</label>
          <TInput type="email" name="email" v-model="user.email" :errors="errors" placeholder="Email Address" />
        </div>
        <div>
          <label for="password" class="pl-.5 font-medium"> Password</label>
          <TInput type="password" name="password" v-model="user.password" :errors="errors" placeholder="Password" />
        </div>
        <div>
          <label for="password_confirmation" class="pl-.5 font-medium"> Comfirm Password</label>
          <TInput type="password"  name="password_confirmation" v-model="user.password_confirmation" :errors="errors"
            placeholder="Confirm Password" inputClass="rounded" />
        </div>
      </div>
      <div>
