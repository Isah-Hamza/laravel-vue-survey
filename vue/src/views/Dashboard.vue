<template>
  <PageComponent title="Dashboard">
    <div v-if="loading" class="flex justify-center flex-col items-center mt-32">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 text-gray-700">
      ...
      <DashboardCard class="order-3 lg:order-1 row-span-2" style="animation-delay: 0.2s">
        <template v-slot:title> <span class="text-2xl font-semibold">Latest Survey</span></template>
        ...
        <div v-else class="text-gray-600 text-center py-16 flex-col flex gap-3 mt-5 ">
          Your don't have surveys yet
          <button @click="handleNavigate('/surveys/create')"
            class="text-sm px-7 py-2 rounded bg-green-600 text-white w-fit mx-auto">New Survey</button>
        </div>
      </DashboardCard>
